...
parser.add_argument('--no_headlines', action='store_true',
                                        help='do not include any headlines')
parser.add_argument('--headline_file', type=str, default='headlines.txt',
                                        help='location of headline file')
parser.add_argument('--generate_tries', type=int, default=1,
                                        help='number of times to do each generation')
parser.add_argument('--output_prefix', type=str, default='output_',
                                        help='prefix to output json')
...
headline_file_name = args.headline_file
generate_tries = args.generate_tries
...
genre_prompts = ('Politics Title:',
                 'Conspiracy Title:', 
                 'Science Title:',
                 'Links https://edition.cnn.com/%Y/%M/%D/business/%titleurl/index.html',
                 'Links https://www.etonline.com/%D/%M/%Y/%titleurl',
                 'Links https://www.dailymail.co.uk/health/article-%number/%titleurl.html',
#                 'Links https://www.nationalenquirer.com',
                 'Links https://www.foxnews.com/sports/%titleurl',
                 'Links https://www.independent.co.uk/news/world/%titleurl-%number.html',
                 'Links https://www.rt.com/news/%number-%titleurl/') 

prefixes = []

headlinelist = ['']
if not no_headlines:
  print('Reading headlines from', headline_file_name)
  headlinelist = open(headline_file_name, 'r')

for headline in headlinelist:
  headline = headline.strip()
  for genre_prompt in genre_prompts:
    index = genre_prompts.index(genre_prompt)
    if not genre_prompt.startswith('Links '):
      prefixes.append( (genre_prompt + ' ' + headline, index) )
    else:
      prefix = str(genre_prompt)
      titleurl = re.sub('[^0-9a-z]', '-', headline.lower())
      prefix = prefix.replace('%titleurl', titleurl)
      number = str(np.random.randint(1000000,9999999))
      prefix = prefix.replace('%number', number)

      if '%Y' in genre_prompt:
        for era in ('past', 'future', 'present'):
          if era == 'future':
            year = 2020
          elif era == 'present':
            year = 2019 
          elif era == 'past':
            year = np.random.randint(2010,2019)
          time_prefix = str(prefix)
          time_prefix = time_prefix.replace('%Y', str(year))
          time_prefix = time_prefix.replace('%M', '%02d' % np.random.randint(1,13))
          time_prefix = time_prefix.replace('%D', '%02d' % np.random.randint(1,30))
          prefixes.append( (time_prefix, str(index) + era) )
      else:
        prefixes.append( (prefix, index) )

outfile = open(output_prefix + '.jsonl', 'w')

for prefix in prefixes:
  # this headline should generate a number of results (10) for a number of sources
  print(prefix)
  prompt = prefix[0].split('\\n') # split on newlines if provided
  ...
  if not any(split_prompt[0] == x for x in CONTROL_CODES.keys()):
    print("WARNING! Generation does not start from a control code so results won't be good")

  # paragraph generation starts here

  for tries in range(generate_tries):
    ...
    try:
      ...
      print(tokens_generated_so_far)
      print()
      json.dump({'prefix':prefix[0], 'genre':prefix[1], 'text':tokens_generated_so_far, 'temperature':temperature, 'nucprob':nucleusprob}, outfile)
      outfile.write("\n")
      
    except KeyboardInterrupt: #Exception as e:
      print('Interrupted: moving to next generation')
